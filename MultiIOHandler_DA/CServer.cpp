...
CServer::CServer(std::string ip, int unsigned port)
    ...
    //: m_IP(ip), m_Port(port), m_acceptor(m_ioContext, asio::ip::tcp::endpoint(asio::ip::tcp::v4(), port))
{
    ...
    //std::thread th1(&CServer::accept, this);
    //std::thread* th1 = new std::thread(&CServer::accept, this);
    //th1->join();
    //run();
    //accept();
}
